...
class BasePageHelper:
    ...
    def check_page(self):
        ...
        elf.find_element(BasePageLocators.VK_ECOSYSTEM_BUTTON)
    ...
    @allure.step('Нажимаем кнопку с экосистемы')
    ...
    @allure.step('Получение идентификатора окна по индексу')
    def get_windows_id(self, index):
        return self.driver.window_handles[index]

    @allure.step('Переключение на окно с ID')
